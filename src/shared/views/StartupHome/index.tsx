        ...
        if (user.status === ApplicationStatus.OPENED && isEmpty(metadata?.profile)) {
            ...
            return (
                <React.Fragment>
                    ...
                    {user.status === ApplicationStatus.REOPENED && <Button className="mt-[30px] h-[38px] w-[143px]" disabled={loading} htmlType="submit" loading={loading} onClick={() => navigate("/startup-type/1")} >
                        {language?.buttons?.continue}
                    </Button>}
                </React.Fragment>
            );
